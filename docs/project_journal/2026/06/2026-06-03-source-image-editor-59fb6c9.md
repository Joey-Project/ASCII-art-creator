...
- PR review hardening: preserved deferred free-rotation frames across repeated rotations, kept final clipped frames padded with transparency, fit free-rotation bounds under the editor canvas budget, ignored stale async upload decodes, cancelled pending upload decodes when edits are cancelled, blocked replacement uploads while edits are open, kept rotation drags aligned to the current canvas center, allowed new crops to grow in every drag direction, preserved hand-tuned grids for unchanged confirmed sources, kept in-flight generation valid when edits are only opened then cancelled, kept Generate disabled while a cancelled edit leaves generation in flight, and allowed retrying the same upload file after cancelling the source editor.
